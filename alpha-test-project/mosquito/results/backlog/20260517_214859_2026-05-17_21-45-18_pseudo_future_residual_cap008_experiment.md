...
## SkillLog 좋았던 점
- Keeping cap008 as a full run was useful even though hit rate fell, because the report now shows mean-distance and threshold-metric disagreement.

## 불편한 점
- Report `Key Findings` stayed TODO, so the cap008 negative result must be manually summarized in a separate backlog.

## 개선점
- Let report generation draft findings from leaderboard deltas and metric tradeoffs, especially when a run improves one metric but hurts the main metric.
