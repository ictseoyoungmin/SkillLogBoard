...
day: "day1"
slice: "04_mode_default_view_policy"
title: "project/run mode default view policy"
priority: "P0"
status: "completed"
target_version: "v1.2"
---

# 04_mode_default_view_policy — project/run mode default view policy

## Objective

Set correct default view depending on whether the board is watching a project root or a single run.
...
1. Expose watch mode in `/api/config` if not already present.
2. Set project mode default view to `overview`.
3. Set single-run mode default view to `lab` or `run` according to existing UI constraints.
4. Document default behavior.
5. Add tests for config/default view behavior.

## Acceptance Criteria

- Project mode opens Overview by default.
- Single-run mode does not pretend to be project overview.
- Docs explain defaults.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_live_server.py tests/test_live_ui_snapshot.py
```
