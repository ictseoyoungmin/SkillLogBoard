...
day: "day2"
slice: "01_view_scoped_state_contract"
title: "view-scoped state contract"
priority: "P0"
status: "completed"
target_version: "v1.2"
---

# 01_view_scoped_state_contract — view-scoped state contract

## Objective

Add view-scoped state loading so the Live Board can avoid reading all data for every screen.
...
1. Support `/api/state?view=overview`, `/api/state?view=compare`, `/api/state?view=artifacts`, `/api/state?view=agent` or equivalent.
2. Keep plain `/api/state` backward-compatible.
3. Return summary-only payload for Overview.
4. Return selected data only for Compare/Lab.
5. Add tests for all view modes.

## Acceptance Criteria

- View-scoped state exists or route-specific equivalent exists.
- Default state remains backward-compatible.
- Overview payload excludes full metric series unless explicitly needed.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_live_server.py tests/test_live_project.py tests/test_live_state.py
```
