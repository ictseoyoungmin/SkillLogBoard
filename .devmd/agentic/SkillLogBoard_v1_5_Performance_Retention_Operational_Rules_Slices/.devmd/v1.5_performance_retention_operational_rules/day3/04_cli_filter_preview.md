...
day: "day3"
slice: "04_cli_filter_preview"
title: "CLI filter preview"
priority: "P0"
status: "completed"
target_version: "v1.5"
---

# 04_cli_filter_preview — CLI filter preview

## Objective

Add CLI support to preview run filters.
...
1. Add `skilllog runs list PROJECT_DIR --filter ...` or similar.
2. Print matching run count and key columns.
3. Support JSON output if feasible.
4. Add tests.

## Acceptance Criteria

- CLI can preview filtered runs.
- JSON output exists or is documented as deferred.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_cli_query.py
```
...
- B1: skilllog runs list PROJECT_DIR --filter EXPR added.
- B2: CLI supports JSON output with count and run records.
- B3: tests/test_v15_index_query.py verifies JSON filtered run listing.
