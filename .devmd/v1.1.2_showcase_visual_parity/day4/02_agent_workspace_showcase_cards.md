...
day: "day4"
slice: "02_agent_workspace_showcase_cards"
title: "agent workspace showcase cards"
priority: "P0"
status: "completed"
target_version: "v1.1.2-showcase-visual-parity"
---

# 02_agent_workspace_showcase_cards — agent workspace showcase cards

## Objective

Make agent evidence visible as a polished local-evidence panel.
...
1. Show action count, latest action, handoff status, decisions status, and files changed in compact cards.
2. Add no-agent empty state with command hints.
3. Avoid summarizing long handoff content.
4. Add tests for agent evidence UI.

## Acceptance Criteria

- Agent panel looks useful when evidence exists.
- No-agent state is clean and helpful.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_live_ui_snapshot.py tests/test_live_state.py
```
