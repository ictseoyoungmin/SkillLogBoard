...
# 03_guided_empty_states — guided empty states

## Objective

Replace generic empty states with action-oriented guidance.
...
1. Update no-metrics empty state with action guidance.
2. Update no-compare empty state with project-watch guidance.
3. Update no-artifacts empty state with report/rich-demo guidance.
4. Update no-agent evidence empty state with agent workflow guidance.
5. Keep text concise.
6. Add snapshot tests for key guidance phrases.

## Acceptance Criteria

- Empty states are actionable.
- Text does not imply unsupported cloud or import features.
- Tests pass.
