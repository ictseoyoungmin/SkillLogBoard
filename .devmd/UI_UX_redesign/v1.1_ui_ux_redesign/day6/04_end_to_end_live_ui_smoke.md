...
day: "day6"
slice: "04_end_to_end_live_ui_smoke"
title: "end-to-end live UI smoke"
priority: "P0"
status: "completed"
target_version: "v1.1-ui-ux-redesign"
---

# 04_end_to_end_live_ui_smoke — end-to-end live UI smoke

## Objective

Run an end-to-end smoke test with a demo run and the redesigned UI/state.
...
1. Ensure `examples/live_demo.py` creates a run with multiple metrics/events.
2. Serve `/` and `/api/state` from a demo-like fixture.
3. Verify HTML contains v1.1 landmarks.
4. Verify state API contains metric catalog and chart marker fields if implemented.
5. Do not require browser/GPU.

## Acceptance Criteria

- Demo run still works.
- Server serves redesigned HTML.
- State API returns expected v1.1 fields.
- No browser/GPU required.
- Tests pass.
...
pytest -q tests/test_live_server.py tests/test_cli_live.py tests/test_live_state.py
```
