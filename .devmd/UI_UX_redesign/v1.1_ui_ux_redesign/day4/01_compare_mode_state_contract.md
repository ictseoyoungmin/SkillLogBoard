...
day: "day4"
slice: "01_compare_mode_state_contract"
title: "compare mode state contract"
priority: "P0"
status: "completed"
target_version: "v1.1-ui-ux-redesign"
---

# 01_compare_mode_state_contract — compare mode state contract

## Objective

Define state/API support for run overlay and compare mode.
...
1. Inspect project state and leaderboard-lite data.
2. Define compare candidates: current, best, latest, selected/pinned runs.
3. Expose bounded metric series for selected compare runs.
4. Add max compare runs and max points limits.
5. Preserve single-run API compatibility.
6. Add tests with multiple runs and shared metric names.

## Acceptance Criteria

- Compare candidate metadata exists.
- Multiple run metric series can be represented/fetched.
- Payload remains bounded.
- Single-run compatibility is preserved.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_live_project.py tests/test_live_server.py tests/test_live_state.py
```
