...
day: "4"
slice: "01_agent_check_result_schema"
title: "agent check result schema"
priority: "P0"
status: "pending"
target_version: "v0.8-agent-research"
---

# 01_agent_check_result_schema — agent check result schema

## Objective

Define structured results for agent completion checks.

## Context

Agent checks should be machine-readable and terminal-friendly.

## Dependencies

- day3 handoff/action CLI completed.
...
1. Create `checks.py`.
2. Define `AgentCheckResult` dataclass or dict schema.
3. Fields: `check_id`, `name`, `outcome`, `severity`, `message`, `details`.
4. Define outcomes: `passed`, `warning`, `error`, `skipped`.
5. Add serialization helper.
6. Add tests for result construction and serialization.

## Acceptance Criteria

- Agent check result schema exists.
