...
day: "day4"
slice: "04_agent_evidence_reader_refinement"
title: "agent evidence reader refinement"
priority: "P0"
status: "completed"
target_version: "v1.1.1-ui-polish-compare"
---

# 04_agent_evidence_reader_refinement — agent evidence reader refinement

## Objective

Improve local agent evidence discovery for the Agent Workspace panel.
...
1. Read `agent/actions.jsonl`, `agent/decisions.md`, and `agent/handoff.md` metadata.
2. Expose latest action status, latest command if present, handoff modified time, and decision file existence.
3. Do not parse or summarize long handoff text in the Live Board by default.
4. Render compact agent summary and link/path affordances.
5. Add tests with and without agent files.

## Acceptance Criteria

- Agent Workspace is more informative when agent files exist.
- No-agent runs still render cleanly.
- Long text is not inlined.
- Tests pass.

## Verification Commands

```bash
pytest -q tests/test_live_state.py tests/test_live_readers.py tests/test_live_ui_snapshot.py
```
