...
day: "day1"
slice: "03_compare_query_parameters"
title: "compare query parameters"
priority: "P0"
status: "completed"
target_version: "v1.1.1-ui-polish-compare"
---

# 03_compare_query_parameters — compare query parameters

## Objective

Support compare query parameters through the local server without breaking default project state.
...
1. Add optional query parameters to `/api/state` in project mode, such as `compare_metric`, `compare_runs`, `max_compare_runs`, and `max_points`.
2. Keep `/api/state` without query params working as before.
3. Validate or sanitize run IDs supplied by query params.
4. Avoid arbitrary path traversal; only choose from discovered local run dirs.
5. Add tests for default project state, selected metric, selected runs, and invalid run IDs.

## Acceptance Criteria

- `/api/state` remains backward-compatible.
- Compare query params return bounded compare data.
- Invalid run IDs do not crash and do not escape project root.
- Tests pass.
