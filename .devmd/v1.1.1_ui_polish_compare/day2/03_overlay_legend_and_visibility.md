...
day: "day2"
slice: "03_overlay_legend_and_visibility"
title: "overlay legend and run visibility"
priority: "P0"
status: "completed"
target_version: "v1.1.1-ui-polish-compare"
---

# 03_overlay_legend_and_visibility — overlay legend and run visibility

## Objective

Add an overlay legend with per-run visibility toggles.
...
1. Render legend entries for each selected compare run.
2. Assign stable color tokens to runs without external libraries.
3. Allow per-run visibility toggling in frontend state.
4. Show hidden/visible state clearly.
5. Ensure legend works in Metric Lab as well as main workspace if feasible.
6. Add tests for legend markup and visibility state strings.

## Acceptance Criteria

- Overlay legend exists.
- Per-run visibility state is represented.
- Run colors are stable within a session.
- Metric Lab can reuse overlay legend or clearly delegates to main legend.
- Tests pass.
