...
day: "day2"
slice: "04_compare_alignment_normalized_raw"
title: "compare alignment and raw/normalized execution"
priority: "P0"
status: "completed"
target_version: "v1.1.1-ui-polish-compare"
---

# 04_compare_alignment_normalized_raw — compare alignment and raw/normalized execution

## Objective

Make compare alignment and raw/normalized controls do real transformation work where possible.
...
1. Implement step and relative alignment for overlay data.
2. Implement raw vs normalized y values for compare overlay.
3. For log scale, handle non-positive values safely with warning or floor value.
4. Ensure smoothing applies per run, not across concatenated runs.
5. Document limitations for unavailable wall-clock/epoch alignment.

## Acceptance Criteria

- Relative alignment works for compare runs.
- Normalized overlay works or is explicitly disabled with docs.
- Smoothing is per-run.
- Non-positive log values do not crash rendering.
- Tests pass.
