...
day: "day5"
slice: "01_scoped_localstorage_key"
title: "scoped localStorage key"
priority: "P0"
status: "completed"
target_version: "v1.1.1-ui-polish-compare"
---

# 01_scoped_localstorage_key — scoped localStorage key

## Objective

Prevent UI preference leakage across unrelated runs and projects.
...
1. Expose stable target identity in `/api/config`, such as mode and target_dir.
2. Compute localStorage key after config load: `skilllogboard.live.ui.v1::{mode}::{target_hash_or_path}`.
3. Keep backward compatibility by ignoring or migrating old global key if simple.
4. Document that preferences are local to browser and scoped by watched target.
5. Add tests for dynamic storage key code and reset behavior strings.

## Acceptance Criteria

- Preferences are scoped per run/project target.
- Reset only affects current scoped preferences.
- No server-side session storage is added.
- Docs explain scoped local preferences.
- Tests pass.
